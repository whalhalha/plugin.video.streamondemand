...
sito = "http://streamingpopcorn.com/portal/"
...
def mainlist(item):
    ...
    itemlist = []
    itemlist.append( Item(channel=__channel__, title="[COLOR azure]Ultimi Film Inseriti[/COLOR]", action="peliculas", url="http://streamingpopcorn.com/portal/?internet", thumbnail="http://orig03.deviantart.net/6889/f/2014/079/7/b/movies_and_popcorn_folder_icon_by_matheusgrilo-d7ay4tw.png"))
...
def peliculas(item):
    ...
    for scrapedurl,scrapedtitle in matches:
        ...
        itemlist.append( Item(channel=__channel__, action="play", fulltitle=scrapedtitle, show=scrapedtitle, title="[COLOR azure]"+scrapedtitle+"[/COLOR]" , url=sito+scrapedurl , thumbnail=scrapedthumbnail , plot=scrapedplot , folder=True) )

    # Extrae el paginador
    patronvideos  = '<h4 class="widgettitle">\s*<a href="(.*?)">Pagina successiva</a>'
...
def play( item ):
    ...
    url = urlparse.urljoin(sito,path)
